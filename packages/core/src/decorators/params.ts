...
export type ParamSchemaBinding<T extends ZodTypeAny> = { index: number; schema: T }
...
export function getParamSchemaBindings(
  target: object,
  key: string | symbol
): ParamSchemaBinding<ZodTypeAny>[] {
  return (Reflect.getMetadata(META_PARAM_SCHEMA, target, key) ||
    []) as ParamSchemaBinding<ZodTypeAny>[]
}
